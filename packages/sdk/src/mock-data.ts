import type { ClinicSearchResult, Location, Service, Testimonial } from './types';
...
export const mockClinicSearchResults: ClinicSearchResult[] = mockLocations.map((loc, i) => ({
  id: loc.id,
  name: loc.name,
  address: loc.address,
  city: loc.city,
  ...
  zipCode: loc.zipCode,
  phone: loc.phone,
  distanceMiles: 2.4 + i * 3.1,
  therapyTypes: ['Physical Therapy', 'Occupational Therapy', 'Speech Therapy'],
}));
...
export function mockFetch<T>(value: T, ms?: number): Promise<T> {
